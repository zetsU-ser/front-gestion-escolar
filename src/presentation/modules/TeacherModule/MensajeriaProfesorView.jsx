...
import { MainContainer, HeaderContainer, TitleText } from '../MensajeriaModule/MensajeriaView.styles';
...
export const MensajeriaProfesorView = () => {
  ...
  // Filtrar cursos y alumnos correspondientes al profesor autenticado
  ...
  useEffect(() => {
    const fetchAsignaciones = async () => {
      ...
      try {
        ...
        // Filtrar asignaciones que correspondan a los cursos del profesor
        const asignacionesProfesor = asignaciones.filter(asig => asig.curso && misCursosIds.includes(asig.curso.id));
        // Mapear al objeto alumno y añadirle el cursoId para uso interno
        const listaAlumnos = asignacionesProfesor.map(asig => ({
          ...asig.alumno,
          cursoId: asig.curso.id
        })).filter(Boolean);
        
        // Quitar duplicados por si un alumno está en múltiples cursos (raro pero posible)
  ...
  const alumnosOpciones = misAlumnos.map(a => ({
    value: a.id,
    ...
    <MainContainer>
      <HeaderContainer>
        ...
        <Typography variant="body2" color="textSecondary" sx={{ mt: 1 }}>
          Envíe notificaciones a los apoderados de sus cursos correspondientes.
        </Typography>
      </HeaderContainer>
      ...
      <FormularioMensajeriaGlobal
        ...
        alcancesOpciones={ALCANCES_PROFESOR}
